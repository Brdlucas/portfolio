        ...
        <div className='h-full m-auto'>
            ...
            <form onSubmit={handleSubmit} className='mt-5 m-auto w-[50%]  max-lg:w-[90%]'>
                <div className='grid grid-cols-1 gap-10 m-auto text-center drop-shadow-2xl rounded-[15px] pt-16 pb-6 border-[3px] border-gray-300 '>
                    <input className={`w-[70%] text-black hover:border-[3px] hover:border-purple-500 outline-purple-500  m-auto h-[50px] focus:rounded-[15px] pl-3 hover:cursor-pointer hover:rounded-[15px] hover:scale-110 duration-400 ${isTheme.theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-white-50'}`} type="text" placeholder="Prénom" value={firstname} onChange={(e) => setFirstname(e.target.value)} required />
                    ...
                    <input className={`w-[70%] text-black hover:border-[3px] hover:border-purple-500 outline-purple-500  m-auto h-[50px] focus:rounded-[15px] pl-3 hover:cursor-pointer hover:rounded-[15px] hover:scale-110 duration-400 ${isTheme.theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-white-50'}`} type="text" placeholder="Entreprise" value={company} onChange={(e) => setCompany(e.target.value)} required />
                    <input className={`w-[70%] text-black hover:border-[3px] hover:border-purple-500 outline-purple-500  m-auto h-[50px] focus:rounded-[15px] pl-3 hover:cursor-pointer hover:rounded-[15px] hover:scale-110 duration-400 ${isTheme.theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-white-50'}`} type="text" placeholder="Poste" value={job} onChange={(e) => setJob(e.target.value)} required />
                    <textarea className={`w-[70%] resize-none pl-3 text-black hover:border-[3px] hover:border-purple-500 outline-purple-500 m-auto h-[150px] focus:rounded-[15px] hover:cursor-pointer hover:rounded-[15px] hover:scale-110 duration-400 ${isTheme.theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-white'}`} placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} />
            ...
            <div className={`mt-3 mb-5`} >
                ...
                <div className={`w-[30%] max-lg:w-[50%] max-md:w-[90%] mt-3 m-auto flex justify-between ${isTheme.theme === 'dark' ? 'bg-gray-700' : ''}`}>
