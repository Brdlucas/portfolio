...
export default function Projects() {
    ...
    return (
        <div>
            ...
            <div>
                <div>
                    <textarea name="search-textarea" placeholder='Rechercher' value={search} onChange={(e) => setSearch(e.target.value)} className={`text-black pt-5 mt-5 flex  m-auto text-center font-semibold resize-none w-[70%] ${isTheme.theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-gray-100 '}`} />
                </div>
                <section className="m-auto w-[95%] pt-10">
                    ...
                    {loading ? <Skeleton /> :
                        <div className='md:pl-[50px] mt-5 max-md:pt-10 grid grid-cols-3 gap-20 max-md:gap-10 max-lg:grid-cols-1 max-md:text-center mb-5 max-2xl:grid-cols-2'>
                            {filterProjects.map((project: any) => (
                                <div key={project._id} className={`block m-auto drop-shadow-2xl shd w-[400px] max-md:w-[300px] h-full rounded-[25px] hover:rounded-[15px] hover:scale-110 duration-400 transition ease-in-out delay-150 ${isTheme.theme === 'dark' ? 'bg-gray-700 ' : 'bg-gray-100'}`}>
